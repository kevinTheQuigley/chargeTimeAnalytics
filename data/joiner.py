...
print("Joining the weather csvs together from multiple stations \n")
...
#weatherHead = weatherHead[(weatherHead['date2'] >= '2019-04-01') & (weatherHead['date2'] < '2022-06-28')]
weatherHead = weatherHead[(weatherHead['date2'] >= '2019-04-01')]
...
print("\n \n Joining multiple years worth of electricity usage data together \n \n")
...
#Old method of reseting column names shown below
#combined.columns = ['Date&Time','ActualGenerationMW','Region','ForecastDemand','ActualWind','Region2','ActualDemand','ForecastDemand','Region3']

#New mothod shown here
...
combined = combined.drop('Region',1)
combined = combined.drop('Region2',1)
combined = combined.drop('Region3',1)

#-------------------Investigating missing values----------------

combMiss = len (pd.date_range(start = combined.date2.min(), end = combined.date2.max(), freq = 'H').difference(combined.date2))
weatMiss = len (pd.date_range(start = weatherHead.date2.min(), end = weatherHead.date2.max(), freq = 'H').difference(weatherHead.date2))

print("\n There are ",weatMiss,"rows missing from the historical weather dataset and ",combMiss," rows missing from the electricity usage dataset \n ")

combined = combined.drop_duplicates(subset=['date2'], keep='first')
combined.reset_index(drop=True, inplace=True)
f = lambda x: x.reindex(pd.date_range(start = combined.date2.min(), end = combined.date2.max(), freq = 'H', name='date2') )

df2 = (combined.set_index('date2')
        .apply(f)
        .reset_index())
misVal=len(df2[df2.ActualGenerationMW.isnull()])
print('\n There are ',misVal,' entries missing from the dataframe. These will be replaced using seasonal Adjustment and interpolation \n')

df2['ActualGenerationMW'] = df2['ActualGenerationMW'].interpolate()
combined = df2
...
print("Datasets succesfully merged and missing columns have been filled")
